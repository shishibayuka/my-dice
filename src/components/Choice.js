...
export const Choice = () => {
    ...
    // {}は処理が１つしかないときは省略している
    const onChoiceCourseClicked = (courseId,courseName) => {
        console.log(`${courseId}${courseName}のゲーム開始します`);
                ...
                <ul>
                    {
                        ...
                        courses.map(course => <li key={course.id}> {course.id}<button type="button" onClick={() =>{onChoiceCourseClicked(course.id,course.name);}}>ゲーム開始 </button>{course.name}</li>)
                    }
                </ul>
            </div>
        </>
